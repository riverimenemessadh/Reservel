<x-app-layout>
    <div class="row fade-in">
        ...
        <div class="col-12 mb-4">
            <h4 class="fw-bold"><i class="fas fa-door-open me-2"></i>{{ __('messages.rooms') }}</h4>
        </div>

        @foreach (['room' => $rooms, 'equipment' => $equipment] as $type => $items)
            @if ($loop->iteration == 2)
                <div class="col-12 mb-4 mt-2">
                    <h4 class="fw-bold"><i class="fas fa-laptop me-2"></i>{{ __('messages.equipment') }}</h4>
                </div>
            @endif
                        ...
                        <div class="cursor-pointer" onclick="window.location.href='{{ route('assets.show', $item) }}'">
                            ...
                            @else
                                <div class="asset-image bg-light d-flex align-items-center justify-content-center"
                                    style="height: 200px;">
                                    <i
                                        class="fas fa-{{ $item->type == 'room' ? 'door-open' : 'laptop' }} fa-4x text-muted"></i>
                                </div>
                            @endif
                            ...
                            <div class="card-body pb-0">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    ...
                                    <span
                                        class="badge bg-{{ $item->status === 'available' ? 'success' : ($item->status === 'in_use' ? 'warning' : 'danger') }}">
                            ...
                            <div class="d-flex gap-2">
                                ...
                                @can('delete', $item)
                                    <button type="button" class="btn btn-sm btn-outline-danger"
                                        ...
                                        onclick="confirmDeleteAsset({{ $item->id }}, '{{ $item->name }}')">
            ...
            @empty
                <div class="col-12 text-center py-4">
                    <p class="text-muted">
                        {{ __('messages.no_assets_available', ['type' => __('messages.' . $type)]) }}
                    </p>
                </div>
            @endforelse
        @endforeach
        ...
        <script>
            function openReportModal(assetId, assetStatus) {
                if (assetStatus === 'in_repair') {
                    // Show warning alert
                    const warningHtml = `
                        <div class="alert alert-warning alert-dismissible fade show" role="alert" style="position: fixed; top: 80px; right: 20px; z-index: 9999; max-width: 400px;">
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content border-0 shadow-lg">
                        <div class="modal-header bg-danger text-white border-0">
                            <h5 class="modal-title"><i class="fas fa-exclamation-triangle me-2"></i> {{ __('messages.delete_asset') }} </h5>
                        ...
                        <div class="modal-footer border-0 justify-content-center">
                            ...
                            <button type="button" class="btn btn-danger px-4" onclick="executeDelete(${assetId})">{{ __('messages.yes_delete') }}</button>
                        </div>
                    </div>
                </div>
            </div>`;
